...
class RecommendationTests {
 ...
 [STAThread] static int Main(){try{
  ...
  string privateFile=Path.Combine(home,"private-feed-test.json"),previousFile=Environment.GetEnvironmentVariable("RIFT_RECOMMENDATIONS_FILE");
  try{Environment.SetEnvironmentVariable("RIFT_RECOMMENDATIONS_FILE",privateFile);File.WriteAllText(privateFile,new JavaScriptSerializer().Serialize(feed));Check(Recommendations.Validate(d,Recommendations.Fetch().GetAwaiter().GetResult(),"Vayne","BOTTOM").Length==1,"Private file feeds existing validation");File.WriteAllText(privateFile,new string('x',2000001));Reject(()=>Recommendations.Fetch().GetAwaiter().GetResult(),"Oversized private file accepted");File.Delete(privateFile);Reject(()=>Recommendations.Fetch().GetAwaiter().GetResult(),"Missing private feed accepted");}finally{Environment.SetEnvironmentVariable("RIFT_RECOMMENDATIONS_FILE",previousFile);if(File.Exists(privateFile))File.Delete(privateFile);}
