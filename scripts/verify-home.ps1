...
foreach($test in @('HomeDataTests','RankHistoryTests','HomeRender','DashboardUiTests','PageNavigationTests','WindowChromeTests','UxInteractionTests','InteractionPerformanceTests')){
 & $compiler /nologo "/out:$appRoot/$test.exe" "/reference:$appRoot/RiftReference.exe" /reference:System.Windows.Forms.dll /reference:System.Drawing.dll /reference:System.Web.Extensions.dll (Join-Path $projectRoot "tests/$test.cs") (Join-Path $projectRoot 'tests/DashboardFixtures.cs')
