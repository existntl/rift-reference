...
public sealed class ChampionsPage : UserControl {
 ...
 readonly TextBox search=new TextBox{Name="ChampionSearch"};readonly RiftComboBox queue=new RiftComboBox{Name="ChampionQueue",AccessibleName="Champion match queue"},sort=new RiftComboBox{Name="ChampionSort",AccessibleName="Sort champions"},role=new RiftComboBox{Name="ChampionRole",AccessibleName="Champion match role"};
 ...
 readonly Label notice;readonly Button ownTab,catalogTab;bool catalog,loaded;
 public ChampionsPage(DataStore data,Func<string,Image> portrait,Action<string> open){this.data=data;this.portrait=portrait;this.open=open;Name="ChampionsPage";BackColor=Theme.Background;
  ...
  ownTab=PageControls.Button(this,"Your champions",24,104,180,()=>{catalog=false;RefreshRows();});catalogTab=PageControls.Button(this,"Champion catalog",216,104,170,()=>{catalog=true;RefreshRows();});
 ...
 void RefreshRows(){rows.SuspendLayout();foreach(Control old in rows.Controls.Cast<Control>().ToArray())old.Dispose();
  ownTab.BackColor=catalog?Theme.Panel:Theme.Accent;ownTab.ForeColor=catalog?Theme.Ink:Theme.Background;catalogTab.BackColor=catalog?Theme.Accent:Theme.Panel;catalogTab.ForeColor=catalog?Theme.Background:Theme.Ink;
...
public sealed class RankHistoryPage : UserControl {
 HomeProfile profile;readonly FlowLayoutPanel rows=new FlowLayoutPanel{FlowDirection=FlowDirection.TopDown,WrapContents=false,AutoScroll=true};readonly RankHistoryPlot plot=new RankHistoryPlot();readonly RiftComboBox range=new RiftComboBox{Name="RankHistoryRange",AccessibleName="LP history period"};
